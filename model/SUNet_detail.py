...
class SUNet(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, out_chans=3,
        ...
        if self.final_upsample == "Dual up-sample":
            ...
            self.output_class = nn.Sequential(nn.Linear(3072,256),
                                        nn.ReLU(),
                                        nn.Linear(256,5),
                                        nn.ReLU())
            
            self.output_re = nn.Conv2d(in_channels=embed_dim, out_channels=self.out_chans, kernel_size=3, stride=1, padding=1, bias=False)
            self.low_conv = nn.Conv2d(in_channels=96, out_channels=3, kernel_size=1)       
